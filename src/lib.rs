...
impl ConstantProduct {
    ...
    pub fn init_wasm(x: u64, y: u64, l: u64, fee: u16, precision: Option<u8>) -> Result<ConstantProduct, JsError> {
        // Assert non-zero values of X and Y
        Ok(ConstantProduct::init(x, y, l, fee, precision)?)
    }
    ...
    // Static Invariant calculation
    #[wasm_bindgen(js_name = k_from_xy)]
    pub fn k_from_xy_wasm(x: u64, y: u64) -> Result<JsValue, JsError> {
        let k = Self::k_from_xy(x, y)?;
        Ok(JsValue::from(k))
    }

    // Get spot price for a token in its opposing token
    #[wasm_bindgen(js_name = k_from_xy_wasm)]
    pub fn spot_price_from_pair_wasm(x: u64, y: u64, precision: u32) -> Result<SpotPriceWasm, JsError> {
        Ok(Self::spot_price_from_pair(x, y, precision)?.into())
    }

    pub fn xy_deposit_amounts_from_l_wasm(x: u64, y: u64, l: u64, a: u64, precision: u32) -> Result<XYAmounts, JsError> {
        Ok(Self::xy_deposit_amounts_from_l(x, y, l, a, precision)?)
    }

    pub fn xy_withdraw_amounts_from_l_wasm(x: u64, y: u64, l: u64, a: u64, precision: u32) -> Result<XYAmounts, JsError> {
        Ok(Self::xy_withdraw_amounts_from_l(x, y, l, a, precision)?)
    }

    pub fn x2_from_y_swap_amount_wasm(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(Self::x2_from_y_swap_amount(x, y, a)?.into())
    }

    pub fn y2_from_x_swap_amount_wasm(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(Self::x2_from_y_swap_amount(y,x,a)?.into())
    }

    pub fn delta_x_from_y_swap_amount_wasm(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(Self::delta_x_from_y_swap_amount(x,y,a)?.into())
    }

    pub fn delta_y_from_x_swap_amount_wasm(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(Self::delta_x_from_y_swap_amount(y,x,a)?.into())
    }

    pub fn k_wasm(&self) -> Result<JsValue, JsError> {
        Ok(Self::k_from_xy(self.x, self.y)?.into())
    }

    pub fn spot_price_x_wasm(&self) -> Result<SpotPriceWasm, JsError> {
        Ok(Self::spot_price_from_pair(self.x, self.y, self.precision)?.into())
    }

    pub fn spot_price_y_wasm(&self) -> Result<SpotPriceWasm, JsError> {
        Ok(Self::spot_price_from_pair(self.y, self.x, self.precision)?.into())
    }

    pub fn swap_unsafe_wasm(&mut self, p: LiquidityPair, a: u64) -> Result<SwapResult, JsError> {
        Ok(self.swap_unsafe(p, a)?)
    }

    pub fn swap_wasm(&mut self, p: LiquidityPair, a: u64, min: u64) -> Result<SwapResult, JsError> {
        Ok(self.swap(p, a, min)?)
    }

    pub fn deposit_liquidity_unsafe_wasm(&mut self, x: u64, y: u64, a: u64) -> Result<DepositLiquidityResult, JsError> {
        Ok(self.deposit_liquidity_unsafe(x, y, a)?)
    }

    pub fn withdraw_liquidity_unsafe_wasm(&mut self, x: u64, y: u64, a: u64) -> Result<WithdrawLiquidityResult, JsError> {
        Ok(self.withdraw_liquidity_unsafe(x, y, a)?)
    }

    pub fn deposit_liquidity_wasm(&mut self, a: u64, max_x: u64, max_y: u64) -> Result<DepositLiquidityResult, JsError> {
        Ok(self.deposit_liquidity(a, max_x, max_y)?)
    }

    pub fn withdraw_liquidity_wasm(&mut self, a: u64, min_x: u64, min_y: u64) -> Result<WithdrawLiquidityResult, JsError> {
        Ok(self.withdraw_liquidity(a, min_x, min_y)?)
    }
}
